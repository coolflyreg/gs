...
require "framework"
local function gen_err_response(self, tag, response, session)
	return function(args)
		header_tmp.type = tag
		header_tmp.session = session
		local header = core.encode(self.__package, header_tmp)
...
function host:dispatch(msg, size, err_name)
	local bin = core.unpack(msg, size)
	header_tmp.type = nil
	...
	local err_proto = nil
	local err_response = nil
	if err_name then
		err_proto = queryproto(self.__proto, err_name)
		err_response = gen_err_response(self, err_proto.tag, err_proto.response, header_tmp.session)
	end

	if header.type and (err_proto == nil or header.type ~= err_proto.tag) then
		-- request
		...
		if header_tmp.session then
			return "REQUEST", proto.name, result, gen_response(self, proto.response, header_tmp.session), err_response
		else
			return "REQUEST", proto.name, result
		end
	elseif (err_proto ~= nil and err_proto.tag == header.type) then
		-- error info response
		...
		if response == true then
			...
			local result = core.decode(err_proto.response, content)
